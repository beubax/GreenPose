...
from models.EfficientVIT import efficientvit_backbone_b1
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    #     {'params': get_parameters_conv(net.nineteenchannel, 'weight')},
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(len(total_losses) // 2):
                losses.append(l2_loss(out["heatmaps"], keypoint_maps, keypoint_masks, images.shape[0]))
                losses.append(l2_loss(out["pafs"], paf_maps, paf_masks, images.shape[0]))
