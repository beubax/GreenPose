...
def infer(net, img, scales, base_height, stride, pad_value=(0, 0, 0), img_mean=(128, 128, 128), img_scale=1/256):
    ...
    for ratio in scales_ratios:
        scaled_img = cv2.resize(normed_img, (256, 192), interpolation=cv2.INTER_CUBIC)
        # min_dims = [base_height, max(scaled_img.shape[1], base_height)]
        # padded_img, pad = pad_width(scaled_img, stride, pad_value, min_dims)

        tensor_img = torch.from_numpy(scaled_img).permute(2, 0, 1).unsqueeze(0).float().cuda()
        out = net(tensor_img)

        stage2_heatmaps = out["heatmaps"]
